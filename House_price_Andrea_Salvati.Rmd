...
weights<- data.table(information.gain(price ~ ., training))
...
weights <- data.table(information.gain(price ~ ., data = training[, price := training$price]))
weights$feature <- rownames(weights)
weights[order(weights$attr_importance, decreasing = TRUE),]

ig_training <- training[, price := training$price]
ig_test <- test[, price := training$price]
ig_validation <- validation[, price := training$price]
...
weights$feature <- rownames(weights)
weights[order(weights$attr_importance, decreasing = TRUE),]
information_gain_features <- weights$feature[weights$attr_importance > ig_cross_validation_threshold]

weights<- data.frame(information.gain(price ~ ., data = training[, price := training$price]))
...
ggplot(my_data, aes(predicted, observed)) +
    geom_point() + geom_smooth(method = "glm") +
    labs(x="Predicted") +
    ggtitle(ggtitle(paste("lasso", 'RMSE: ', format(round(lasso.mod.mape, 4), nsmall=4), ' --> Price ERROR:', format(round(lasso.mod.price_error, 0), nsmall=0), 
                        ' €', sep=''))) +  
    scale_x_continuous(labels = scales::comma) + 
    scale_y_continuous(labels = scales::comma)
