...
niche_grade<-names(which(summary(dataset$grade)/nrow(dataset)<0.03))
niche_grade
...
xgb_0<-xgboost(booster='gbtree',
               data=as.matrix(xgb_ig_training[, !'price', with=F]),
               label=xgb_ig_training$price,
               nrounds = 100,
               objective='reg:linear')

xgb.pred<-predict(xgb_0, newdata = as.matrix(xgb_ig_validation[, !'price', with=F]), type='response')

```

```{r XGBoost - plot, warning=FALSE}

xgb.mape <- mape(xgb.pred, xgb_ig_validation$price)
xgb.price_error <- mean(abs((exp(xgb.pred) -1) - (exp(xgb_validation$price) -1)))

my_data <- as.data.frame(cbind(predicted=(exp(xgb.pred) -1), observed=(exp(xgb_ig_validation$price) -1)))
...
final.pred <- as.numeric(exp(predict(final.model, as.matrix(xgb_test[, !'price', with=F]), type='response'))-1)
